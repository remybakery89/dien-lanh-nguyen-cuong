(() => {
  ...
  original.onload = () => {
    ...
    style.textContent = `
      #service-details { width:100%; max-width:1100px; margin:0 auto; padding:72px 16px 80px; box-sizing:border-box; overflow:hidden; }
      #service-details > .section-heading { width:100%; max-width:650px; margin:0 auto 36px; }
      #service-details .service-detail { width:100%; max-width:1050px; min-width:0; margin:0 auto 28px; display:grid; grid-template-columns:minmax(0,.9fr) minmax(0,1.1fr); align-items:stretch; overflow:hidden; border:1px solid #e7e9eb; border-radius:22px; background:#fff; box-shadow:0 10px 30px rgba(23,37,54,.07); box-sizing:border-box; scroll-margin-top:90px; }
      ...
      #service-details .service-detail-media { position:relative; min-width:0; min-height:100%; height:auto; overflow:hidden; background:#eef1f3; }
      #service-details .service-detail-media img { display:block; width:100%; height:100%; min-height:390px; object-fit:cover; object-position:center; }
      #service-details .service-detail-body { min-width:0; padding:38px 42px; display:flex; flex-direction:column; justify-content:center; box-sizing:border-box; overflow-wrap:anywhere; }
      #service-details .service-detail-number { margin:0 0 10px; color:#e8892d; font-size:11px; font-weight:850; letter-spacing:.12em; }
      #service-details .service-detail-body h3 { margin:0 0 14px; color:#172536; font-size:clamp(27px,3vw,38px); line-height:1.05; letter-spacing:-.035em; }
      #service-details .service-detail-intro { margin:0; color:#66717d; font-size:14px; line-height:1.65; }
      #service-details .service-detail-columns { display:grid; grid-template-columns:minmax(0,1fr) minmax(0,1fr); gap:24px; margin-top:24px; padding-top:22px; border-top:1px solid #e7e9eb; }
      #service-details .service-detail-columns h4 { margin:0 0 10px; color:#172536; font-size:13px; line-height:1.3; }
      #service-details .service-detail-columns ul { display:grid; gap:7px; margin:0; padding:0; list-style:none; }
      #service-details .service-detail-columns li,#service-details .service-detail-columns p { margin:0; color:#66717d; font-size:12px; line-height:1.55; }
      #service-details .service-detail-columns li { position:relative; padding-left:14px; }
      #service-details .service-detail-columns li::before { content:''; position:absolute; left:0; top:.58em; width:5px; height:5px; border-radius:50%; background:#e8892d; }
      #service-details .service-detail-handle { margin-top:20px; padding:13px 15px; border-left:3px solid #e8892d; background:#f7f5f1; color:#66717d; font-size:12px; line-height:1.55; }
      #service-details .service-detail-handle strong { color:#172536; }
      #service-details .service-detail-actions { display:flex; flex-wrap:wrap; gap:10px; margin-top:22px; }
      #service-details .service-detail-actions .btn { min-height:44px; padding:0 18px; border-radius:12px; font-size:12px; }
      #service-details .service-detail-actions .btn-secondary { border:1px solid #d9dde1; color:#172536; background:#fff; }
      ...
      @media (max-width:699px) {
        ...
        #service-details .service-detail,#service-details .service-detail:nth-of-type(even) { grid-template-columns:1fr; margin-bottom:18px; border-radius:18px; }
        #service-details .service-detail:nth-of-type(even) .service-detail-media,#service-details .service-detail:nth-of-type(even) .service-detail-body { order:initial; }
    ...
    if (priceSection) {
      const priceData = [
        { title:'Máy lạnh', rows:[['Máy lạnh không hoạt động','300.000 – 800.000đ'],['Máy hoạt động nhưng không lạnh','400.000 – 1.000.000đ'],['Remote bấm không được','150.000 – 400.000đ'],['Máy hoạt động có tiếng kêu lạ','300.000 – 700.000đ'],['Hỏng bo mạch / báo lỗi','500.000 – 1.500.000đ']] },
        { title:'Máy giặt', rows:[['Máy giặt không lên nguồn','300.000 – 700.000đ'],['Máy giặt không vắt','400.000 – 900.000đ'],['Máy bị kêu, run lắc','300.000 – 800.000đ'],['Hư bo mạch / báo lỗi','500.000 – 1.500.000đ']] },
        { title:'Tủ lạnh', rows:[['Tủ không đông đá','400.000 – 1.000.000đ'],['Ngăn mát kém lạnh','300.000 – 800.000đ'],['Tủ hở ron cửa','200.000 – 500.000đ'],['Hư bo mạch / báo lỗi','500.000 – 1.500.000đ'],['Block không hoạt động','800.000 – 2.000.000đ']] },
        { title:'Máy nước nóng', rows:[['Máy không hoạt động','300.000 – 700.000đ'],['Hoạt động nhưng không nóng','400.000 – 900.000đ'],['Máy ra nước yếu','200.000 – 500.000đ'],['Hư bo mạch / báo lỗi','500.000 – 1.200.000đ']] },
        { title:'Lò vi sóng', rows:[['Lò không hoạt động','300.000 – 700.000đ'],['Hoạt động nhưng không nóng','400.000 – 900.000đ'],['Bị liệt bàn phím','200.000 – 500.000đ'],['Lò hư bo mạch / mất nguồn','500.000 – 1.200.000đ']] }
      ];
      ...
      priceSection.innerHTML = `
        ...
        <div class="price-cards-v2">
          ${priceData.map((group,index)=>`<article class="price-card-v2"><div class="price-card-head"><span class="price-card-number">0${index+1}</span><h3>${group.title}</h3></div><div class="price-table-v2" role="table" aria-label="Bảng giá ${group.title}"><div class="price-table-head"><span>Tình trạng thường gặp</span><strong>Giá tham khảo</strong></div>${group.rows.map(row=>`<div class="price-table-row"><span>${row[0]}</span><strong>${row[1]}</strong></div>`).join('')}</div></article>`).join('')}
        </div>
        <div class="price-transparency-v2">
          <div class="price-transparency-copy"><p class="eyebrow-dark">BẠN LUÔN BIẾT TRƯỚC CHI PHÍ</p><h3>Kiểm tra trước – báo giá trước – <span>khách đồng ý mới sửa</span></h3><p>Giá tham khảo có thể thay đổi tùy tình trạng thiết bị, model, mức độ hư hỏng và linh kiện thực tế cần thay thế. Nguyễn Cường sẽ trao đổi rõ để bạn cân nhắc trước khi quyết định.</p></div>
          <div class="price-transparency-steps" aria-label="Quy trình báo giá"><span><b>01</b> Kiểm tra thực tế</span><i aria-hidden="true">→</i><span><b>02</b> Xác định nguyên nhân</span><i aria-hidden="true">→</i><span><b>03</b> Báo giá</span><i aria-hidden="true">→</i><span><b>04</b> Khách đồng ý mới sửa</span></div>
        </div>
        <div class="price-cta-v2"><div><strong>Đang cần biết máy nhà mình sửa hết bao nhiêu?</strong><span>Gửi tình trạng hoặc hình ảnh thiết bị, Nguyễn Cường tư vấn trước để bạn dễ hình dung.</span></div><div class="price-cta-actions"><a class="btn btn-primary" href="tel:${PHONE}">☎ GỌI NGAY</a><a class="btn btn-secondary" href="${ZALO}" target="_blank" rel="noopener">Zalo CHAT</a></div></div>
      `;
      ...
      priceStyle.textContent = `
        ...
        .price-heading-v2 h2 { color:#fff; }
        .price-heading-v2 h2 span { color:#e8892d; }
        .price-heading-v2 > p:last-child { color:rgba(255,255,255,.78); }
        .price-heading-v2 strong { color:#e8892d; }
        ...
        @media (max-width:899px) { .price-cards-v2 { grid-template-columns:1fr; } .price-cta-v2 { align-items:flex-start; flex-direction:column; } }
        @media (max-width:699px) {
          .price-cards-v2 { margin-top:28px; gap:14px; }
          .price-card-v2 { border-radius:17px; }
          .price-card-head { padding:17px; }
      ...
      const revealPrice=()=>{ priceSection.querySelectorAll('.reveal').forEach(element=>{ const rect=element.getBoundingClientRect(); if(rect.top<window.innerHeight*.96&&rect.bottom>0) element.classList.add('visible'); }); };
